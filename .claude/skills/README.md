...
## Running Commands
Always wrap multi-step commands using bash -c:
```bash
bash -c 'source "$HOME/.sdkman/bin/sdkman-init.sh" && cd "/home/user/Desktop/Orient Advertise/claude-ai-spring-boot" && ./gradlew clean build --no-daemon 2>&1 | grep -B1 "FAILED" | head -10'
```